...
class BoundedRetryPolicy:
    """Resume durable checkpoints and retry model strategy within explicit budgets."""
    ...
    def _executor_runs(self, task_id: str) -> list[dict]:
        return [
            run for run in self.runtime.list_runs(task_id) if run["role"] == "EXECUTOR"
        ]
    ...
    def _active_workspace(self, task_id: str) -> dict | None:
        ...
        if len(active) > 1:
            raise RuntimeInvariantError(f"task {task_id} has multiple active workspaces")
    ...
    def _transition_after_resume_failure(self, task_id: str, target: TaskStatus) -> None:
        task = self.runtime.get_task(task_id)
        if task["status"] == TaskStatus.RUNNING.value:
            self.runtime.transition_task(
                task_id,
    ...
    def _finish_resumed_success(
        ...
        *,
        ...
        self._record_policy_verification(
            task_id,
            status="PASS",
            reason="resumed durable workspace completed required verification",
    ...
    def _resume_sandbox(self, task_id: str, workspace: dict) -> PolicyResult:
        ...
        try:
            ...
            self._transition_after_resume_failure(task_id, TaskStatus.BLOCKED)
            return PolicyResult(
                task_id,
        ...
        self._transition_after_resume_failure(task_id, TaskStatus.FAILED)
        return PolicyResult(
            task_id,
    ...
    def _resume_applied(self, task_id: str, workspace: dict) -> PolicyResult:
        ...
        if proposal_id is None:
            return self._quarantine(
                task_id,
                "applied workspace has no durable proposal linkage",
                workspace_id=workspace_id,
            )
    ...
    def _budget_exhausted(self, task_id: str) -> str | None:
        ...
        if (
            ...
            and verification_failures >= config.max_verification_failures
        ):
            return (
    ...
    def drive(
        ...
        *,
        ...
        while True:
            ...
            had_executor_attempts = bool(self._executor_runs(task_id))
            model = self._select_model(task_id)
            action = PolicyAction.RETRY if had_executor_attempts else PolicyAction.ATTEMPT
            ...
            if last_attempt.stage == AttemptStage.WORKSPACE:
                return PolicyResult(
                    task_id,
                    PolicyOutcome.FAILED,
                    PolicyAction.STOP,
            ...
            # Executor blocking and deterministic model/apply/audit/verification
            # failures are strategy outcomes. The next loop iteration may retry,
            # but only while the explicit durable budgets still permit it.

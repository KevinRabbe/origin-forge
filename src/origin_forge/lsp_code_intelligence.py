...
class LspCodeIntelligenceProvider:
    ...
    This class does not spawn a process and does not expose raw LSP requests to
    a model. The caller owns session/process policy. Every file URI returned by
    the server is revalidated through the Workspace mapper before the result is
        ...
        *,
        ...
        if self.mapper.workspace_root != repository.project_root:
            raise CodeIntelligenceError(
                "LSP mapper root must match RepositoryReader root"
            )
    ...
    def _query_position(self, path: str, position: TextPosition) -> dict:
        ...
        if position.line >= len(lines):
            raise CodeIntelligenceError(
                f"LSP query line is outside {path}: {position.line}"
            )
        try:
        ...
        *,
        ...
        for item in raw:
            ...
            if not isinstance(location, dict) or "range" not in location:
                continue
            mapped_kind = _SYMBOL_KINDS.get(kind, SymbolKind.VARIABLE)
            result.append(
                CodeSymbol(
                    name,
                    mapped_kind,
    ...
    def definitions(
        ...
        *,
        ...
        raw = self._request(
            ...
            self._query_position(path, position),
        )
        if raw is None:
            return ()
        values = raw if isinstance(raw, list) else [raw]
        result = [self._definition_location(item) for item in values[:limit]]
        return tuple(result)

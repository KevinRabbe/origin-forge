...
def _score(descriptor: ToolDescriptor, query: str, terms: set[str]) -> int:
    ...
    if normalized_query:
        normalized_id = " ".join(
            _tokens(descriptor.tool_id.replace(".", " ").replace("-", " "))
        )
...
class ToolSearchSession:
    ...
    def __init__(
        ...
        *,
        ...
        max_describes: int = 32,
        max_query_chars: int = 1024,
        max_query_terms: int = 32,
        max_search_description_chars: int = 320,
    ):
        if not isinstance(view, AuthorizedToolView):
            raise TypeError("view must be an AuthorizedToolView")
        for value, name in (
            (max_searches, "max_searches"),
            (max_results_per_search, "max_results_per_search"),
            (max_hydrated_tools, "max_hydrated_tools"),
            (max_describes, "max_describes"),
        ...
        self.max_describes = max_describes
    ...
    def describe_tool(self, tool_id: str) -> dict[str, object]:
        ...
        try:
            ...
            raise ToolAccessDenied(
                f"tool is not available in this authority scope: {tool_id}"
            ) from exc

        if self._describes >= self.max_describes:
            raise ToolSearchBudgetExceeded(
                f"tool describe budget exhausted ({self._describes} >= {self.max_describes})"
            )

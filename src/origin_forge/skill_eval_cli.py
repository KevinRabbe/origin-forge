...
def build_parser() -> argparse.ArgumentParser:
    ...
    add_case.add_argument("--fixture-ref", required=True)
    add_case.add_argument("--scorer-ref", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
            ...
            _print(
                {
                    ...
                    "path": path.relative_to(runtime.project_root).as_posix(),
                }
            )
            return 0
        ...
        if args.command == "report-show":
            loaded = store.load_report(args.report_id)
            _print(loaded.envelope)
            return 0

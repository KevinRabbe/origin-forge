...
BLENDER_PRODUCTION_TASK_ACCEPTANCE_MIGRATION = Migration(
    20,
    r"""
CREATE TABLE blender_production_task_acceptances (
    execution_id TEXT PRIMARY KEY REFERENCES blender_production_adoptions(execution_id) ON DELETE RESTRICT,
...
CREATE INDEX idx_blender_production_task_acceptances_task
ON blender_production_task_acceptances(task_id, accepted_at, execution_id);

CREATE TRIGGER blender_production_task_acceptances_immutable_update
BEFORE UPDATE ON blender_production_task_acceptances
BEGIN
    SELECT RAISE(ABORT, 'blender production task acceptances are immutable');
END;

CREATE TRIGGER blender_production_task_acceptances_immutable_delete
BEFORE DELETE ON blender_production_task_acceptances
BEGIN
    SELECT RAISE(ABORT, 'blender production task acceptances are immutable');
END;
""",

        ...
        result.append(
            LspServerConfig(
                ...
                probe_timeout_seconds=_positive_float(
                    item, "probe_timeout_seconds", 10.0, label
                ),
                initialize_timeout_seconds=_positive_float(
                    item, "initialize_timeout_seconds", 15.0, label
                ),
                request_timeout_seconds=_positive_float(
                    item, "request_timeout_seconds", 5.0, label
                ),
                shutdown_timeout_seconds=_positive_float(
                    item, "shutdown_timeout_seconds", 5.0, label
                ),
                max_protocol_message_bytes=_positive_int(
                    item, "max_protocol_message_bytes", 4 * 1024 * 1024, label
                ),
                max_pending_notifications=_positive_int(
                    item, "max_pending_notifications", 256, label
                ),
                max_stderr_bytes=_positive_int(
                    item, "max_stderr_bytes", 256 * 1024, label
                ),
            )
        )
    return tuple(result)

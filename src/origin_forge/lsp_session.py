...
class LspJsonRpcSession:
    ...
    def _write_message(self, payload: dict) -> None:
        ...
        encoded = encode_lsp_message(payload, max_message_bytes=self.max_message_bytes)
    ...
    def _queue_response(self, message: dict) -> None:
        try:
            ...
            raise LspProtocolError("LSP pending response limit exceeded") from exc
        ...
        try:
            while not self._closed:
                ...
                if message.get("jsonrpc") != "2.0":
                    raise LspProtocolError("LSP message must declare jsonrpc='2.0'")
                ...
                if isinstance(method, str):
                    ...
                    with self._notification_lock:
                        if len(self._notifications) >= self.max_pending_notifications:
                            raise LspProtocolError("LSP pending notification limit exceeded")
                        if (
                ...
                if "id" not in message:
                    raise LspProtocolError("LSP response is missing both method and id")

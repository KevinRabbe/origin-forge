...
class SkillBenchmarkRunner:
    ...
    The runner owns pairing and aggregation only. The injected trial callable
    owns model/tool execution and external scoring. Baseline and candidate
    receive the same deterministic seed for each case/repetition. Execution
    order alternates by repetition to reduce systematic order/cache bias.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if max_candidate_skills <= 0:
            raise ValueError("max_candidate_skills must be positive")
    ...
    def _validate_candidate(self, candidate_skills: Sequence[Skill]) -> None:
        ...
        if len(candidate_skills) > self.max_candidate_skills:
            raise SkillEvaluationError(
                "candidate Skill count exceeds limit "
                f"({len(candidate_skills)} > {self.max_candidate_skills})"
            )
        ...
        *,
        ...
        for case in case_list:
            ...
            for repetition in range(self.repetitions):
                ...
                if repetition % 2 == 0:
                    baseline_results.append(self.trial(baseline_request))
                    candidate_results.append(self.trial(candidate_request))
                else:
                    candidate_results.append(self.trial(candidate_request))
                    baseline_results.append(self.trial(baseline_request))

...
def _require_receipt_policy_relation(runtime: OriginForgeRuntime, receipt: TaskPreparationReceipt):
...
def _require_route_checkpoint_current(runtime: OriginForgeRuntime, receipt: TaskPreparationReceipt, policy) -> None:
...
def _require_work_order_checkpoint_current(runtime: OriginForgeRuntime, receipt: TaskPreparationReceipt) -> None:
    ...
    if (
        currentness.task_id != receipt.task_id
        or receipt.work_order_hash is None
        or receipt.work_order_audit_hash is None
    ):
        raise PreparationReceiptError("PREP WorkOrder checkpoint relation is incomplete")


def _require_bound_checkpoint_current(runtime: OriginForgeRuntime, receipt: TaskPreparationReceipt) -> None:
    ...
    if (
        ...
        raise PreparationReceiptError("BOUND PREP lacks exact Phase-34 authority IDs")
    currentness = inspect_dispatch_binding_currentness_readonly(
        runtime,
        receipt.input_resolution_id,
        receipt.dispatch_binding_id,
        receipt.binding_audit_id,
...
def _state_for_current_receipt(receipt: TaskPreparationReceipt) -> PreparationInspectionState:
    ...
    raise PreparationReceiptError("ACTIVE PREP stage is outside resumable Phase-39 states")

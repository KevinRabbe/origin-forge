...
class PixeloramaProductionProvenanceSigningFailureCode(StrEnum):
...
class PixeloramaProductionProvenanceSigningError(RuntimeError):
    def __init__(
        self,
        code: PixeloramaProductionProvenanceSigningFailureCode,
        detail: str,
    ) -> None:
        self.code = code
...
class PixeloramaProductionProvenanceSigningBlocked(
...
class GovernedPixeloramaProductionProvenanceResult:
    execution_id: str
    task_id: str
...
def _require_current_adopted_png(
    runtime: OriginForgeRuntime,
    *,
    ...
    for part in relative.parts:
        ...
        if current.is_symlink():
            raise PixeloramaProductionProvenanceSigningBlocked(
    ...
    if inspection.byte_count != accepted_byte_count:
        raise PixeloramaProductionProvenanceSigningBlocked(
            PixeloramaProductionProvenanceSigningFailureCode.ADOPTED_ARTIFACT_DRIFT,
            "canonical adopted Pixelorama Artifact is not current",
        )
...
class GovernedPixeloramaProductionProvenanceSigner:
    """Sign one terminally accepted Pixelorama production Artifact without production authority."""
    ...
    def sign(
        ...
        *,
        ...
        if not isinstance(certificate_id, str) or not validate_id(
            ...
            raise PixeloramaProductionProvenanceSigningBlocked(
                PixeloramaProductionProvenanceSigningFailureCode.PROVENANCE_TRUST_NOT_READY,
        ...
        try:
            ...
            adoption = read_pixelorama_production_adoption_receipt(self.runtime, execution_id)
            acceptance = read_pixelorama_production_task_acceptance(self.runtime, execution_id)
            ...
            raise PixeloramaProductionProvenanceSigningBlocked(
                PixeloramaProductionProvenanceSigningFailureCode.ADOPTED_ARTIFACT_DRIFT,
                "canonical Pixelorama production relation cannot be resolved",
            ) from exc
        ...
        if (
            ...
            or acceptance.acceptance_authority != PRODUCTION_TASK_ACCEPTANCE_AUTHORITY
            ...
            raise PixeloramaProductionProvenanceSigningBlocked(
                PixeloramaProductionProvenanceSigningFailureCode.ADOPTED_ARTIFACT_DRIFT,
                "canonical Pixelorama production relation is not exact",
            )

        _require_current_adopted_png(
            self.runtime,
        ...
        if (
            ...
            or manifest.artifact_type != "SPRITESHEET_EXPORT"
            ...
            raise PixeloramaProductionProvenanceSigningError(
                PixeloramaProductionProvenanceSigningFailureCode.SIGNED_MANIFEST_CONFLICT,
                "new signed provenance manifest does not bind the exact Pixelorama production relation",
            )

        inspection = self.provenance_service.verify_manifest(manifest.manifest_id)
        if not inspection.trusted_and_current:
            raise PixeloramaProductionProvenanceSigningError(
                PixeloramaProductionProvenanceSigningFailureCode.SIGNED_MANIFEST_NOT_CURRENT,
        ...
        return GovernedPixeloramaProductionProvenanceResult(

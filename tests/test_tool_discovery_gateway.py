...
class ToolDiscoveryGatewayTests(unittest.TestCase):
    ...
    def test_describe_response_hydrates_one_authorized_contract(self) -> None:
        ...
        self.assertEqual(tool_payload["authority_hash"], self.session.view.authority_hash)
        self.assertEqual(self.gateway.status().describes_used, 1)
    ...
    def test_unknown_operation_and_extra_arguments_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ToolDiscoveryGatewayError, "do not match"):
            self.gateway.call(
                "search_tools",
                {"query": "read file", "unexpected": True},
            )
        with self.assertRaisesRegex(ToolDiscoveryGatewayError, "must be an integer"):
            self.gateway.call(
                "search_tools",
    ...
    def test_status_contains_only_discovery_provenance_and_budgets(self) -> None:
        ...
        self.assertEqual(status["searches_used"], 1)
        self.assertEqual(status["describes_used"], 1)

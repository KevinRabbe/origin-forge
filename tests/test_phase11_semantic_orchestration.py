...
from origin_forge.orchestration import AttemptOutcome, BoundedTaskOrchestrator
...
class SequencedSandbox:
    backend_id = "phase11-semantic-test-sandbox"
...
class Phase11SemanticOrchestrationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.runtime.initialize("phase11-semantic-integration")
...
''',
    ...
    def _ready_task(self) -> str:
        goal = self.runtime.create_goal("Phase 11 semantic integration")
        flow = self.runtime.create_flow(goal)
        self.runtime.transition_flow(flow, FlowStatus.RUNNING, expected_revision=0)
        task = self.runtime.create_task(
            flow,
            "Repair WidgetParser while changing build_widget behavior",
        )
    ...
    def test_one_shot_semantic_expansion_reaches_executor_context(self) -> None:
        task = self._ready_task()
        model = RecordingModel(self._response(value="semantic"), "model-a")
    ...
    def test_retry_reselects_semantic_context_in_each_fresh_workspace(self) -> None:
        task = self._ready_task()
        ...
        first_paths = tuple(
            item["path"] for item in first.requests[0].context["files"]
        )
        second_paths = tuple(
            item["path"] for item in second.requests[0].context["files"]
        )
        self.assertEqual(first_paths, second_paths)
        self.assertEqual(first_paths[0], "src/service.py")

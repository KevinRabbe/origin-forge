...
from origin_forge.playtest_cli import build_parser, main
from origin_forge.runtime import OriginForgeRuntime
from origin_forge.state import FlowStatus, RunStatus, TaskStatus


class PlaytestCliTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tempdir = tempfile.TemporaryDirectory()
        self.root = Path(self.tempdir.name)
        self.runtime = OriginForgeRuntime(self.root)
        self.runtime.initialize("playtest-cli-test")
    ...
    def _playtest_run(self) -> str:
        goal = self.runtime.create_goal("Playtest")
        flow = self.runtime.create_flow(goal)
        self.runtime.transition_flow(flow, FlowStatus.RUNNING, expected_revision=0)
        task = self.runtime.create_task(flow, "Run synthetic player")
        ...
        run_id = self.runtime.start_run(task, role="PLAYTESTER")
    ...
    def test_status_is_deterministic_and_non_mutating(self) -> None:
        ...
        self.assertEqual(value["playtest_run_count"], 0)
        self.assertEqual(value["artifact_counts"], {})
        self.assertFalse(value["playtest_execution_enabled"])
        self.assertFalse(value["raw_os_input_enabled"])
        self.assertFalse(value["scenario_mutation_enabled"])
        self.assertFalse(value["task_mutation_enabled"])
        self.assertFalse(value["semantic_game_quality_authority_enabled"])
        self.assertFalse(value["canonical_asset_adoption_enabled"])
        self.assertEqual(self.runtime.status(), before)

    def test_sessions_and_run_show_are_limited_to_playtester_role(self) -> None:
        run_id = self._playtest_run()
        ...
        self.assertEqual(
            shown["verifications"][0]["verification_type"], "playtest-structure"
        )

    def test_artifact_show_is_limited_to_phase24_types(self) -> None:
        ...
        self.assertIn("Phase-24 playtest Artifact", value["detail"])

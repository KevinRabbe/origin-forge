...
def git(root: Path, *args: str) -> None:
    subprocess.run(["git", *args], cwd=root, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)


def location(path: str) -> CodeLocation:
    return CodeLocation(path, TextRange(TextPosition(0, 0), TextPosition(0, 1)))
...
class FakeProvider:
    ...
    def __init__(self, symbols: dict[str, tuple[CodeSymbol, ...]], *, available: bool = True):
        ...
        self.capabilities = CodeIntelligenceCapabilities(workspace_symbols=True, definitions=True, references=True, diagnostics=True)
    ...
    def definitions(self, path, position, *, limit=20): return ()
    def references(self, path, position, *, include_declaration=True, limit=100): return ()
    def diagnostics(self, paths, *, limit_per_file=100): return ()
...
class CodeIntelligenceContextTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "src" / "payments.py").write_text("class PaymentCoordinator:\n    def refund_invoice(self):\n        return True\n", encoding="utf-8")
        (self.root / "tests" / "test_payments.py").write_text("def test_refund_invoice():\n    assert True\n", encoding="utf-8")
        ...
        self.task = self.runtime.create_task(flow, "Fix PaymentCoordinator refund invoice behavior", acceptance_criteria=["refund invoice test passes"], required_capabilities=["code-edit"])
    ...
    def test_task_relevant_workspace_symbol_adds_tracked_file(self) -> None:
        provider = FakeProvider({
            "payment": (CodeSymbol("PaymentCoordinator", SymbolKind.CLASS, location("src/payments.py")),),
            "refund": (
                CodeSymbol("refund_invoice", SymbolKind.METHOD, location("src/payments.py"), "PaymentCoordinator"),
                CodeSymbol("test_refund_invoice", SymbolKind.FUNCTION, location("tests/test_payments.py")),
            ),
        })
        result = CodeIntelligenceContextExpander(self.runtime, self.reader, provider).expand(self.task, ["src/service.py"])
    ...
    def test_untracked_provider_location_is_ignored(self) -> None:
        ...
        provider = FakeProvider({"payment": (CodeSymbol("PaymentCoordinator", SymbolKind.CLASS, location("secret.py")),)})
        result = CodeIntelligenceContextExpander(self.runtime, self.reader, provider).expand(self.task, ["src/service.py"])
    ...
    def test_literal_git_pathspec_does_not_widen_wildcard_like_filename(self) -> None:
        ...
        provider = FakeProvider({"payment": (CodeSymbol("PaymentCoordinator", SymbolKind.CLASS, location("src/payment[1].py")),)})
        result = CodeIntelligenceContextExpander(self.runtime, self.reader, provider).expand(self.task, ["src/service.py"])
    ...
    def test_query_and_file_budgets_are_hard(self) -> None:
        provider = FakeProvider({"refund": (
            CodeSymbol("refund_invoice", SymbolKind.METHOD, location("src/payments.py")),
            CodeSymbol("test_refund_invoice", SymbolKind.FUNCTION, location("tests/test_payments.py")),
        )})
        result = CodeIntelligenceContextExpander(
            ...
            self.reader,
            provider,
            settings=CodeIntelligenceContextSettings(max_queries=1, max_symbols_per_query=1, max_files=2, max_total_bytes=4096),
    ...
    def test_tracked_path_output_budget_is_hard(self) -> None:
        provider = FakeProvider({})
        with self.assertRaisesRegex(CodeIntelligenceContextError, "output exceeds byte limit"):
            CodeIntelligenceContextExpander(
                self.runtime,
                self.reader,
                provider,
                settings=CodeIntelligenceContextSettings(max_git_output_bytes=4),
            ).expand(self.task, ["src/service.py"])
    ...
    def test_unavailable_provider_fails_explicitly(self) -> None:
        with self.assertRaisesRegex(CodeIntelligenceContextError, "unavailable"):
            CodeIntelligenceContextExpander(self.runtime, self.reader, FakeProvider({}, available=False)).expand(self.task, ["src/service.py"])

    def test_untracked_seed_is_rejected(self) -> None:
        self.root.joinpath("notes.py").write_text("VALUE = 1\n", encoding="utf-8")
        with self.assertRaisesRegex(CodeIntelligenceContextError, "not tracked"):
            CodeIntelligenceContextExpander(self.runtime, self.reader, FakeProvider({})).expand(self.task, ["notes.py"])

    def test_repeated_expansion_is_deterministic(self) -> None:
        provider = FakeProvider({"payment": (CodeSymbol("PaymentCoordinator", SymbolKind.CLASS, location("src/payments.py")),)})
        expander = CodeIntelligenceContextExpander(self.runtime, self.reader, provider)

...
from origin_forge.code_diagnostics import CodeDiagnosticsEvaluator, CodeDiagnosticsSettings
...
def diagnostic(path: str, severity: DiagnosticSeverity, message: str, *, line: int = 0) -> CodeDiagnostic:
...
class FakeDiagnosticsProvider:
    ...
    def __init__(self, values=(), *, available=True, diagnostics=True):
        ...
        self.capabilities = CodeIntelligenceCapabilities(diagnostics=diagnostics)
...
class CodeDiagnosticsEvaluatorTests(unittest.TestCase):
    def test_errors_are_visible_without_becoming_runtime_truth(self) -> None:
        ...
        result = CodeDiagnosticsEvaluator(provider).evaluate(["src/a.py", "src/b.py"])
    ...
    def test_warning_only_result_passes(self) -> None:
        provider = FakeDiagnosticsProvider((diagnostic("src/a.py", DiagnosticSeverity.WARNING, "warning"),))
    ...
    def test_diagnostics_and_messages_are_bounded(self) -> None:
        provider = FakeDiagnosticsProvider(tuple(
            diagnostic(f"src/{index}.py", DiagnosticSeverity.INFORMATION, "x" * 100, line=index)
            for index in range(10)
        ))
        result = CodeDiagnosticsEvaluator(
            provider,
            settings=CodeDiagnosticsSettings(max_diagnostics=3, max_message_chars=8),
    ...
    def test_provider_request_budget_is_partitioned_across_paths(self) -> None:
        ...
        CodeDiagnosticsEvaluator(
            provider,
            settings=CodeDiagnosticsSettings(max_diagnostics=5, max_paths=4),
        ).evaluate(["a.py", "b.py"])
    ...
    def test_excessive_path_set_fails_before_provider_request(self) -> None:
        ...
        evaluator = CodeDiagnosticsEvaluator(
            provider,
            settings=CodeDiagnosticsSettings(max_diagnostics=10, max_paths=2),
        )
    ...
    def test_unavailable_or_unsupported_provider_fails_explicitly(self) -> None:
        with self.assertRaisesRegex(CodeIntelligenceError, "unavailable"):
            CodeDiagnosticsEvaluator(FakeDiagnosticsProvider(available=False)).evaluate(["src/a.py"])
        with self.assertRaisesRegex(CodeIntelligenceError, "does not support"):
            CodeDiagnosticsEvaluator(FakeDiagnosticsProvider(diagnostics=False)).evaluate(["src/a.py"])

    def test_error_policy_can_be_evidence_only(self) -> None:
        provider = FakeDiagnosticsProvider((diagnostic("src/a.py", DiagnosticSeverity.ERROR, "error"),))
        result = CodeDiagnosticsEvaluator(
            provider,

...
class LspCodeIntelligenceTests(unittest.TestCase):
    ...
    def test_definition_query_and_result_translate_utf16_positions(self) -> None:
        ...
        session = FakeSession(
            {
                "textDocument/definition": {
                    "uri": self.mapper.path_to_uri("src/b.py"),
                    "range": {
        ...
        self.assertEqual(
            sent["position"]["character"],
            codepoint_to_lsp_character(
                query_line,
                query_position.character,
    ...
    def test_workspace_symbols_are_normalized_and_limited(self) -> None:
        ...
        session = FakeSession(
            {
                "workspace/symbol": [
                    {"name": "Widget", "kind": 5, "location": location},
                    {"name": "helper", "kind": 12, "location": location},
                ]
            }
        )
        provider = LspCodeIntelligenceProvider(
    ...
    def test_references_send_include_declaration_policy(self) -> None:
        location = {
            "uri": self.mapper.path_to_uri("src/a.py"),
            "range": {
                "start": {"line": 0, "character": 0},
                "end": {"line": 0, "character": 1},
            },
        ...
        provider = LspCodeIntelligenceProvider(
            self.reader,
            session,
            self._capabilities(position_encoding=LspPositionEncoding.UTF8),
            mapper=self.mapper,
        )
        ...
        self.assertEqual(len(result), 1)
        sent = session.requests[0][1]
        self.assertEqual(sent["context"], {"includeDeclaration": False})
    ...
    def test_unadvertised_capability_is_not_queried(self) -> None:
        ...
        provider = LspCodeIntelligenceProvider(
            self.reader,
            session,
            self._capabilities(definitions=False),
            mapper=self.mapper,
        )

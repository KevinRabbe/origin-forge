...
class ToolCatalogTests(unittest.TestCase):
    ...
    def test_catalog_rejects_bad_types_duplicate_ids_and_count_overflow(self) -> None:
        tool = descriptor("repo.read_file")
        with self.assertRaisesRegex(ToolCatalogError, "ToolDescriptor"):
            ToolCatalogSnapshot.create([tool, object()])
    ...
    def test_authority_hash_is_order_independent_but_set_sensitive(self) -> None:
        read = descriptor("repo.read_file")
        search = descriptor("repo.search_text", capability="repo.search")
        catalog = ToolCatalogSnapshot.create([read, search])
        left = AuthorizedToolView.create(
            catalog,
            ["repo.read_file", "repo.search_text"],
        )
        reordered = AuthorizedToolView.create(
            catalog,
            ["repo.search_text", "repo.read_file"],
        )
        subset = AuthorizedToolView.create(catalog, ["repo.read_file"])
        self.assertEqual(left.authority_hash, reordered.authority_hash)
        self.assertNotEqual(left.authority_hash, subset.authority_hash)

    def test_authority_rejects_unknown_invalid_and_duplicate_ids_before_search(self) -> None:
        ...
        with self.assertRaisesRegex(ToolCatalogError, "invalid authorized tool ID"):
            AuthorizedToolView.create(catalog, ["Repo Read"])
        with self.assertRaisesRegex(ToolCatalogError, "duplicate tool IDs"):
            AuthorizedToolView.create(
                catalog,
                ["repo.read_file", "repo.read_file"],
            )

    def test_metadata_validation_returns_catalog_errors_for_bad_values(self) -> None:
        with self.assertRaisesRegex(ToolCatalogError, "non-empty strings"):

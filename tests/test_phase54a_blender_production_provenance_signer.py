...
from origin_forge.production_blender_provenance_signer import (
    BlenderProductionProvenanceSigningBlocked,
    BlenderProductionProvenanceSigningFailureCode,
...
NOW = "2026-08-22T19:30:00Z"


class Phase54ABlenderProductionProvenanceSignerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _accepted(self):
        runtime, binding, adoption, _, _ = self._published()
        accepted = GovernedBlenderProductionTaskAcceptor(runtime).accept(
            binding.execution_id,
            actor_id="operator.phase54a.accept",
        )
    ...
    def _provenance(self, runtime):
        ...
        if openssl is None:
            self.skipTest("OpenSSL required for Phase 54A signing tests")
    ...
    def test_malformed_execution_fails_before_provenance_signing(self) -> None:
        runtime, _, _, _, _ = self._published()
        signer = GovernedBlenderProductionProvenanceSigner(runtime)
        with self.assertRaises(BlenderProductionProvenanceSigningBlocked) as caught:
            signer.sign(
    ...
    def test_adopted_but_not_terminally_accepted_execution_cannot_be_signed(self) -> None:
        runtime, binding, _, _, _ = self._published()
        signer = GovernedBlenderProductionProvenanceSigner(runtime)
        with self.assertRaises(BlenderProductionProvenanceSigningBlocked) as caught:
            signer.sign(
                binding.execution_id,
    ...
    def test_terminal_acceptance_signs_exact_adopted_artifact_without_production_mutation(self) -> None:
        ...
        result = GovernedBlenderProductionProvenanceSigner(
            runtime,
        ...
        self.assertEqual(manifest.artifact_type, "BLENDER_GLB_EXPORT")
    ...
    def test_post_acceptance_glb_drift_preserves_acceptance_but_blocks_signing(self) -> None:
        runtime, binding, adoption, _ = self._accepted()
        service, certificate, operational_key, _ = self._provenance(runtime)
    ...
    def test_two_explicit_signing_invocations_create_distinct_manifests_only(self) -> None:
        ...
        artifact_id = adoption.adopted_artifact_id
        self.assertIsNotNone(artifact_id)
        signer = GovernedBlenderProductionProvenanceSigner(
            runtime,
            provenance_service=service,
        )
        before = self._production_snapshot(runtime, binding.task_id, artifact_id)

...
class PythonCodeIntelligenceTests(unittest.TestCase):
    ...
    def test_definition_resolves_symbol_from_usage(self) -> None:
        ...
        definitions = self.provider.definitions("src/pkg/service.py", position)
    ...
    def test_references_include_declaration_and_usage(self) -> None:
        ...
        references = self.provider.references(
            "src/pkg/models.py", position, include_declaration=True
        )
        ...
        without_declaration = self.provider.references(
            "src/pkg/models.py", position, include_declaration=False
        )
    ...
    def test_untracked_python_is_not_in_workspace_symbols(self) -> None:
        ...
        self.assertEqual(self.provider.workspace_symbols("SecretWidgetParser"), ())
    ...
    def test_git_enumeration_output_limit_fails_closed(self) -> None:
        provider = PythonAstCodeIntelligence(
            self.reader,
            settings=PythonIntelligenceSettings(max_git_output_bytes=8),
        )
        with self.assertRaisesRegex(CodeIntelligenceError, "byte limit"):

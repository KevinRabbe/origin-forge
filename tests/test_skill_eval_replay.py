...
class SkillEvalReplayTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.case = SkillEvalCase(
            case_id="parser",
            fixture_ref="git-fixture:parser-v1",
    ...
    def test_current_inputs_are_replayable(self) -> None:
        ...
        self.assertEqual(status.protocol_id, "paired-skill-ab-v1")
        self.assertEqual(status.environment_fingerprint, "env:model-a+harness-v1")

    def test_changed_live_skill_makes_historical_report_stale_not_invalid(self) -> None:
        stored = self._stored_report()
    ...
    def test_report_tampering_is_detected_before_replayability(self) -> None:
        stored = self._stored_report()
    ...
    def test_suite_hash_tampering_is_detected_even_with_new_valid_report_id(self) -> None:
        stored = self._stored_report()
        raw = json.loads(stored.path.read_text(encoding="utf-8"))
        ...
        report_id = f"SKILL-EVAL-{digest[:20]}"
        path = self.store.reports_dir / f"{report_id}.json"
        path.write_bytes(canonical)
        with self.assertRaisesRegex(SkillEvalStoreError, "suite hash mismatch"):
            SkillEvalReplayInspector(self.runtime).inspect(report_id)

    def test_unsupported_protocol_is_rejected_even_when_content_address_is_valid(self) -> None:
        stored = self._stored_report()
        raw = json.loads(stored.path.read_text(encoding="utf-8"))
        raw["report"]["protocol_id"] = "future-protocol"
        canonical = (
            json.dumps(raw, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
            + "\n"
        ).encode("utf-8")
        ...
        digest = hashlib.sha256(canonical).hexdigest()
        report_id = f"SKILL-EVAL-{digest[:20]}"
        self.store.reports_dir.joinpath(f"{report_id}.json").write_bytes(canonical)
        with self.assertRaisesRegex(SkillEvalStoreError, "unsupported Skill benchmark protocol"):
            SkillEvalReplayInspector(self.runtime).inspect(report_id)

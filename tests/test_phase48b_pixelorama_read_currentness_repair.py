...
class Phase48BPixeloramaReadCurrentnessRepairTests(unittest.TestCase):
    ...
    def test_readonly_nonzero_ref_promotion_is_exact_not_generic(self) -> None:
        ...
        self.assertIn("pixelorama_input = (", source)
        ...
        self.assertIn('resolved[0].original_ref.ref_type.value == "ARTIFACT"', source)
        self.assertIn('resolved[0].original_ref.role == "pixelorama_project"', source)
        self.assertIn('resolved[0].source_object_type == "ARTIFACT"', source)
        self.assertIn('resolved[0].resolution_class == "CANONICAL_ARTIFACT"', source)
        self.assertIn("blender_input = (", source)
        self.assertIn('binding.selected_adapter_id == "originforge.blender.model3d"', source)
        self.assertIn('binding.dispatch_contract_id == "blender.export-glb@1"', source)
        self.assertIn('binding.binder_id == "binder.blender.export-glb@1"', source)
        self.assertIn('resolved[0].original_ref.ref_type.value == "MODEL3D_REQUEST"', source)
        self.assertIn('resolved[0].original_ref.role == "model3d_request"', source)
        ...
        self.assertIn("if not (pixelorama_input or blender_input):", source)

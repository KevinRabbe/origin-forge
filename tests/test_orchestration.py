...
class BoundedOrchestrationTests(unittest.TestCase):
    ...
    def _model_for_update(self, expected_hash: str | None = None) -> FakeModel:
        expected = expected_hash or RepositoryReader(self.root).hash_file("hello.py")
        return FakeModel(
            json.dumps(
                {
    ...
    def test_successful_attempt_completes_verified_pipeline_once(self) -> None:
        ...
        self.assertEqual(
            (self.workspaces.path(result.workspace_id) / "hello.py").read_text(
                encoding="utf-8"
            ),
            "print('new')\n",
        )
    ...
    def test_executor_reads_workspace_snapshot_not_dirty_user_checkout(self) -> None:
        ...
        expected_hash = f"sha256:{hashlib.sha256(committed).hexdigest()}"
        self.source.write_text("print('user-dirty')\n", encoding="utf-8")
        model = self._model_for_update(expected_hash)

        result = BoundedTaskOrchestrator(
            self.runtime,
            model,
            FakeSandbox([SandboxResult(0, "ok", "", False, 5)]),
            workspaces=self.workspaces,
        ).execute(self.task, selected_paths=["hello.py"])

        self.assertEqual(result.outcome, AttemptOutcome.SUCCEEDED)
        self.assertEqual(model.requests[0].context["files"][0]["content"], "print('old')\n")
        self.assertEqual(
            model.requests[0].context["files"][0]["content_hash"], expected_hash
        )
        self.assertEqual(
            self.source.read_text(encoding="utf-8"), "print('user-dirty')\n"
        )
        self.assertEqual(
            (self.workspaces.path(result.workspace_id) / "hello.py").read_text(
                encoding="utf-8"
    ...
    def test_empty_proposal_blocks_and_abandons_unused_snapshot(self) -> None:
        model = FakeModel(
            json.dumps(
                {
    ...
    def test_preflight_sandbox_unavailable_blocks_before_workspace_or_model(self) -> None:
        model = self._model_for_update()
        result = BoundedTaskOrchestrator(
            self.runtime,
            model,
    ...
    def test_invalid_model_output_fails_task_and_abandons_unused_snapshot(self) -> None:
    ...
    def test_sandbox_infrastructure_error_blocks_and_preserves_audited_workspace(self) -> None:
        model = self._model_for_update()
        result = BoundedTaskOrchestrator(
            self.runtime,
            model,
    ...
    def test_requires_ready_task(self) -> None:
    ...
    def test_existing_active_workspace_must_be_resumed_or_abandoned(self) -> None:
        task_id = self._running_task_for_workspace_preflight()
        ...
        self.assertEqual(model.calls, 0)
        self.assertEqual(
            self.workspaces.get(workspace_id)["status"], WorkspaceStatus.CREATED.value
        )

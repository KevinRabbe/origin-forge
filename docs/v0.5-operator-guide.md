...
Status: **R1 RELEASE-CANDIDATE OPERATOR SNAPSHOT — `0.5.0` VERSION / TAG PENDING**

This guide freezes the operator surface accepted on the Phase-47 mainline for the canonical v0.5 milestone. During R1 the package intentionally remains `0.2.0.dev0`; a later R2 may change release identity wording to `0.5.0` only after this operator boundary and the v0.5 acceptance/readiness records are accepted.
...
R1 candidate metadata still uses package version `0.2.0.dev0` under the Apache License 2.0. The final `0.5.0` version transition is an explicit later release-candidate gate, not an authority change.

...
Status: **v0.5.0 FINAL RELEASE CANDIDATE — TAG PENDING**

This guide freezes the operator surface accepted on the Phase-47 mainline for the final `0.5.0` candidate. R1 accepted this command/authority boundary while retaining the development version; R2 changes only release identity to `0.5.0`. The release remains tag-pending until immutable CI/review/merge proof is complete.

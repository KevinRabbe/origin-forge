...
This guide describes the intended local operator surface for Origin Forge v0.1.0. It is deliberately narrower than the complete library/API surface.
...
The final release candidate uses package version `0.1.0` under the Apache License 2.0. The `v0.1.0` tag remains downstream of exact-head Python 3.12/3.13 CI, clean review/thread state, and SHA-guarded merge of the final release candidate.
...
Origin Forge v0.1.0 is licensed under the Apache License 2.0; see the repository `LICENSE` file. See `docs/v0.1-release-readiness.md` and `docs/v0.1-acceptance-matrix.md` for the release gates and roadmap traceability.

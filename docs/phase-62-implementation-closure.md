...
The source-create vertical now has a dedicated schema-v32 acceptance receipt.
Acceptance requires a validated explicit human actor identity, the published
source-adoption receipt, the exact current Task revision, and an immutable Task
Verification before terminalization. Calls without an actor fail closed without
changing the Task.
